    ...
    def select_file(self):
        ...
        if file_path:
            ...
            self.time_label.config(
                text="00:00:00 / " + self.get_duration_str())
            self.play_video()
    ...
    def get_duration_str(self):
        if self.playing_video:
            ...
            total_duration_str = str(
                timedelta(milliseconds=total_duration))[:-3]
...
#       self.menuBar = mb
        ...
        m.add_separator()
        self.playItem = m.add_item(
            'Play', self.OnPlay, key=Cmd_ + 'P')  # Play/Pause
        m.add_item('Stop', self.OnStop, key=Cmd_ + '\b')  # BackSpace
        m.add_separator()
        m.add_item('Zoom In',  self.OnZoomIn,  key=(
            Cmd_ + 'Shift++') if _3_9 else '')
        m.add_item('Zoom Out', self.OnZoomOut,
                   key=(Cmd_ + '-') if _3_9 else '')
        m.add_separator()
        m.add_item('Faster', self.OnFaster, key=(
            Cmd_ + 'Shift+>') if _3_9 else '')
        m.add_item('Slower', self.OnSlower, key=(
            Cmd_ + 'Shift+<') if _3_9 else '')
        m.add_separator()
...
#       self.videoMenu = m
        ...
        m = _Tk_Menu(mb)  # Video menu, shortcuts to both panels
        m.bind_shortcuts_to(v, b)
        self.anchorItem = m.add_item(
            _UN_ANCHORED, self.OnAnchor, key=Cmd_ + 'A')
        m.add_separator()
        self.opaqueItem = m.add_item(_OPACITY % (
            self._opacity,), self.OnOpacity, key=Cmd_ + 'Y')
...
#       Event Types <https://www.Tcl.Tk/man/tcl8.6/TkCmd/bind.html#M7>
        # window resize, position, etc.
        p.bind('<Configure>',       self.OnConfigure)
...
#           self.buttonsPanel.title(t)
    ...
    def _set_buttons_title(self, *tensecs):
        ...
        if s and T:
            ...
            t = '%s - %s / %s%s%s' % (self._title, t,
                                      s, self._scaleXstr, self._ratestr)
...
if __name__ == '__main__':  # MCCABE 13
    ...
    while len(sys.argv) > 1:
        ...
        if arg in ('-v', '--version'):
            ...
            print(
                'usage: %s  [-v | --version]  [-debug]  [<video_file_name>]' % (_argv0,))
            sys.exit(1)
        elif arg:  # video file
    ...
    if _isWindows:  # see function _test() at the bottom of ...
        ...
        root.mainloop()  # forever
>>>>>>> fb9d8405fb15f665607b9c46454f8c2c1eabd3f1
